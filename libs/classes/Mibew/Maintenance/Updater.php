...
class Updater
{
    ...
    protected function setDatabaseVersion($version)
    {
        ...
        try {
            return $db->query(
                'UPDATE {chatconfig} SET vcvalue = :version WHERE vckey = :key LIMIT 1',
                array(
                    ':version' => $version,
                    ':key' => 'dbversion',
                )
    ...
    protected function update20000Beta4()
    {
        ...
        try {
            $operators = $db->query(
                'SELECT operatorid AS id, vcavatar AS avatar FROM {chatoperator}',
                null,
            ...
            foreach ($operators as $operator) {
                ...
                $db->query(
                    'UPDATE {chatoperator} SET vcavatar = :avatar WHERE operatorid = :id',
                    array(
                        ':id' => $operator['id'],
                        ':avatar' => $matches[1],
                    )
    ...
    protected function update20100()
    {
        ...
        try {
            // Alter locale table.
            $db->query('ALTER TABLE {chatlocale} ADD COLUMN name varchar(128) NOT NULL DEFAULT "" AFTER code');
            $db->query('ALTER TABLE {chatlocale} ADD COLUMN rtl tinyint NOT NULL DEFAULT 0');
            $db->query('ALTER TABLE {chatlocale} ADD COLUMN time_locale varchar(128) NOT NULL DEFAULT "en_US"');
            $db->query('ALTER TABLE {chatlocale} ADD COLUMN date_format text');

            $db->query('ALTER TABLE {chatlocale} ADD UNIQUE KEY code (code)');

            // Create a table for available updates.
            $db->query('CREATE TABLE {chatavailableupdate} ( '
        ...
        try {
            ...
            $locales = $db->query(
                'SELECT localeid as id, code from {chatlocale}',
                null,
                array('return_rows' => Database::RETURN_ALL_ROWS)
            );
    ...
    protected function update20200()
    {
        ...
        try {
            // Alter plugin table.
            $db->query('ALTER TABLE {chatplugin} ADD COLUMN initialized tinyint NOT NULL DEFAULT 0 AFTER enabled');
        } catch (\Exception $e) {
            $this->errors[] = getlocal('Cannot update tables: {0}', $e->getMessage());
    ...
    protected function update30100()
    {
        ...
        try {
            ...
            $db->query('ALTER TABLE {chatrevision} ADD PRIMARY KEY (id)');
        } catch (\Exception $e) {
            $this->errors[] = getlocal('Cannot update tables: {0}', $e->getMessage());
    ...
    protected function update30200()
    {
        ...
        try {
            ...
            $db->query('ALTER TABLE {chatrequestcallback} ADD COLUMN `func` VARCHAR(64) NOT NULL AFTER `function`');
            $db->query('UPDATE {chatrequestcallback} SET `func` = `function`');
            $db->query('ALTER TABLE {chatrequestcallback} DROP COLUMN `function`');
        } catch (\Exception $e) {
            $this->errors[] = getlocal('Cannot update tables: {0}', $e->getMessage());
