...
class Installer
{
    ...
    protected function prepopulateDatabase()
    {
        ...
        try {
            ...
            if ($count == 0) {
                ...
                $db->query(
                    'UPDATE {chatconfig} SET vcvalue = :value WHERE vckey = :key',
                    array(
                        ':key' => '_instance_id',
                        ':value' => Utils::generateInstanceId(),
                    )
    ...
    protected function importLocalesContent()
    {
        ...
        try {
            $locales = $db->query(
                'SELECT * FROM {chatlocale} WHERE enabled = :enabled',
                array(':enabled' => 0),
                array('return_rows' => Database::RETURN_ALL_ROWS)
            );
            ...
            foreach ($locales as $locale_info) {
                ...
                // is imported.
                $db->query(
                    'UPDATE {chatlocale} SET enabled = :enabled WHERE code = :locale',
                    array(
                        ':locale' => $locale,
                        ':enabled' => 1,
                    )
