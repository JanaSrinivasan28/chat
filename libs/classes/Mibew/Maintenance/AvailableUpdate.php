...
class AvailableUpdate
{
    ...
    public static function load($id)
    {
        ...
        $info = Database::getInstance()->query(
            "SELECT * FROM {chatavailableupdate} WHERE id = :id",
            array(':id' => $id),
    ...
    public static function loadByTarget($target)
    {
        ...
        // Load update info
        $info = Database::getInstance()->query(
            "SELECT * FROM {chatavailableupdate} WHERE target = :target",
            array(':target' => $target),
            array('return_rows' => Database::RETURN_ONE_ROW)
        );
    ...
    public static function all()
    {
        $rows = Database::getInstance()->query(
            "SELECT * FROM {chatavailableupdate}",
            null,
    ...
    public function delete()
    {
        ...
        Database::getInstance()->query(
            "DELETE FROM {chatavailableupdate} WHERE id = :id LIMIT 1",
            array(':id' => $this->id)
        );
    }
    ...
    public function save()
    {
        ...
        if (!$this->id) {
            // This update is new.
            $db->query(
                ("INSERT INTO {chatavailableupdate} (target, version, url, description) "
            ...
            $db->query(
                ("UPDATE {chatavailableupdate} SET target = :target, url = :url, "
