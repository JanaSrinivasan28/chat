...
class HistoryController extends AbstractController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        if ($query !== false) {
            ...
            $groups = $db->query(
                ("SELECT {chatopgroup}.groupid AS groupid, vclocalname " .
                    "FROM {chatopgroup} " .
    ...
    protected function buildAccessCondition($operator)
    {
        ...
        if (!is_capable(CAN_VIEWTHREADS, $operator)) {
            return array(
                'condition' => ' AND {chatthread}.agentid = :operator_id ',
                'values' => array(
        ...
        if (in_isolation($operator)) {
            ...
            if (count($operators_placeholders) > 0) {
                ...
                $query_conditions[] = '{chatthread}.agentid IN (' . $operators_in_statement . ')';
            ...
            if (count($groups_placeholders) > 0) {
                ...
                $query_conditions[] = '{chatthread}.groupid IN (' . $groups_in_statement . ')';
