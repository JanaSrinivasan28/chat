...
class State
{
    ...
    public static function load($id)
    {
        ...
        $info = Database::getInstance()->query(
            'SELECT * FROM {chatplugin} WHERE id = :id',
            array(':id' => $id),
    ...
    public static function loadByName($name)
    {
        ...
        // Load plugin state
        $info = Database::getInstance()->query(
            'SELECT * FROM {chatplugin} WHERE name = :name',
            array(':name' => $name),
            array('return_rows' => Database::RETURN_ONE_ROW)
        );
    ...
    {
        $rows = Database::getInstance()->query(
            'SELECT * FROM {chatplugin} WHERE enabled = :enabled AND installed = :installed',
            array(
                ':enabled' => 1,
                ':installed' => 1,
            ),
    ...
    public function save()
    {
        ...
        if (!$this->id) {
            // This state is new.
            $db->query(
                ("INSERT INTO {chatplugin} (name, version, installed, enabled, initialized) "
            ...
            $db->query(
                ("UPDATE {chatplugin} SET name = :name, version = :version, "
    ...
    public function delete()
    {
        ...
        Database::getInstance()->query(
            "DELETE FROM {chatplugin} WHERE id = :id LIMIT 1",
            array(':id' => $this->id)
        );
    }
