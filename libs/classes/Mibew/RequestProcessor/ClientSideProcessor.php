...
abstract class ClientSideProcessor extends AbstractProcessor
{
    ...
    protected function addRequestToBuffer($key, $request)
    {
        ...
        $db->query(
            "INSERT INTO {chatrequestbuffer} (request, requestkey) VALUES (:request, :key)",
    ...
    protected function getRequestsFromBuffer($key)
    {
        ...
        $requests = $db->query(
            "SELECT request FROM {chatrequestbuffer} WHERE requestkey = :key",
            array(':key' => $key),
            array('return_rows' => Database::RETURN_ALL_ROWS)
        );
        // Remove got requests from database
        $db->query(
            "DELETE FROM {chatrequestbuffer} WHERE requestkey = :key",
            array(':key' => $key)
        );
