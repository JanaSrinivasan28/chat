...
function get_operators_list($options = array())
{
    ...
    $query = "SELECT DISTINCT "
        ...
        . (empty($options['isolated_operator_id'])
            ? ""
            : ", {chatoperatortoopgroup} "
                . "WHERE {chatoperator}.operatorid = {chatoperatortoopgroup}.operatorid "
                . "AND {chatoperatortoopgroup}.groupid IN ("
                    . "SELECT g.groupid FROM {chatopgroup} g, "
                        . "(SELECT {chatopgroup}.groupid, {chatopgroup}.parent FROM {chatopgroup}, {chatoperatortoopgroup} "
...
function update_operator($operator)
{
    ...
    Database::getInstance()->query(
        ('UPDATE {chatoperator} SET vclogin = :login, vcpassword=:password, '
...
function delete_operator($operator_id)
{
    $db = Database::getInstance();
    $db->query(
        "DELETE FROM {chatoperatortoopgroup} WHERE operatorid = ?",
        array($operator_id)
    );
    $db->query(
...
function has_online_operators($group_id = "")
{
    ...
    if ($group_id) {
        $query .= ", {chatoperatortoopgroup}, {chatopgroup} "
            . "WHERE {chatopgroup}.groupid = {chatoperatortoopgroup}.groupid "
                . "AND ({chatopgroup}.groupid = :groupid OR {chatopgroup}.parent = :groupid) "
                . "AND {chatoperator}.operatorid = {chatoperatortoopgroup}.operatorid "
                . "AND istatus = 0";
...
function get_online_operators($group_id = null)
{
    ...
    if (!$group_id) {
        ...
        $groups = $db->query(
            ('SELECT o.* FROM {chatoperator} o, {chatoperatortoopgroup} r, {chatopgroup} g '
