    ...
    if (is_null($available_locales)) {
        if (get_maintenance_mode() !== false) {
            ...
            $rows = Database::getInstance()->query(
                "SELECT code FROM {chatlocale} WHERE enabled = 1",
                array(),
    ...
    if (!isset($cache[$locale])) {
        if (get_maintenance_mode() === false) {
            ...
            $info = Database::getInstance()->query(
                'SELECT * FROM {chatlocale} WHERE code = :code',
                array(':code' => $locale),
...
function set_locale_info($locale, $info)
{
    ...
    $success = Database::getInstance()->query(
        ('UPDATE {chatlocale} SET name = :name, rtl = :rtl, '
...
function load_db_messages($locale)
{
    ...
    $db_messages = $db->query(
        'SELECT * FROM {chattranslation} WHERE locale = ?',
        array($locale),
        array(
...
function save_message($locale, $key, $value)
{
    ...
    if (array_key_exists($key, $available_messages[$locale])) {
        ...
        $db->query(
            ('UPDATE {chattranslation} SET translation = :translation '
        ...
        $db->query(
            ('INSERT INTO {chattranslation} (locale, source, translation, hash) '
...
function enable_locale($locale)
{
    ...
    list($count) = $db->query(
        "SELECT COUNT(*) FROM {chatlocale} WHERE code = :code",
        array(':code' => $locale),
        array(
    ...
    if ($count == 0) {
        ...
        Database::getInstance()->query(
            ('INSERT INTO {chatlocale} (code, enabled) VALUES(:code, :enabled)'),
            array(
                ':code' => $locale,
                ':enabled' => 1,
            )
        ...
        $db->query(
            "UPDATE {chatlocale} SET enabled = :enabled WHERE code = :code",
            array(
                ':enabled' => 1,
                ':code' => $locale,
            )
