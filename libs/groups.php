...
function group_by_id($id)
{
    ...
    $group = $db->query(
        "SELECT * FROM {chatopgroup} WHERE groupid = ?",
        array($id),
...
function group_by_name($name)
{
    $db = Database::getInstance();
    $group = $db->query(
        "SELECT * FROM {chatopgroup} WHERE vclocalname = ?",
        array($name),
        array('return_rows' => Database::RETURN_ONE_ROW)
    );
...
function get_operator_groups_list($operator_id)
{
    ...
    if (Settings::get('enablegroups') == '1') {
        $group_ids = array(0);
        $all_groups = $db->query(
            "SELECT groupid FROM {chatoperatortoopgroup} WHERE operatorid = ? ORDER BY groupid",
            array($operator_id),
...
function get_available_parent_groups($skip_group)
{
    ...
    $groups_list = $db->query(
        ("SELECT {chatopgroup}.groupid AS groupid, parent, vclocalname "
            . "FROM {chatopgroup} ORDER BY vclocalname"),
        null,
        array('return_rows' => Database::RETURN_ALL_ROWS)
    );
...
function group_has_children($group_id)
{
    $db = Database::getInstance();
    $children = $db->query(
        "SELECT COUNT(*) AS count FROM {chatopgroup} WHERE parent = ?",
        array($group_id),
        array('return_rows' => Database::RETURN_ONE_ROW)
    );
...
function update_group($group)
{
    ...
    if ($group['parent']) {
        $db->query(
            "UPDATE {chatopgroup} SET parent = NULL WHERE parent = ?",
            array($group['groupid'])
        );
    }
...
function get_group_members($group_id)
{
    $rows =  Database::getInstance()->query(
        "SELECT operatorid FROM {chatoperatortoopgroup} WHERE groupid = ?",
        array($group_id),
        array('return_rows' => Database::RETURN_ALL_ROWS)
    );
...
function update_group_members($group_id, $new_value)
{
    ...
    $db = Database::getInstance();
    $db->query(
        "DELETE FROM {chatoperatortoopgroup} WHERE groupid = ?",
        array($group_id)
    );

    foreach ($new_value as $operator_id) {
        $db->query(
            "INSERT INTO {chatoperatortoopgroup} (groupid, operatorid) VALUES (?, ?)",
...
{
    $db = Database::getInstance();
    $db->query("DELETE FROM {chatopgroup} WHERE groupid = ?", array($group_id));
    $db->query("DELETE FROM {chatoperatortoopgroup} WHERE groupid = ?", array($group_id));
...
function get_all_groups()
{
    $db = Database::getInstance();
    $groups = $db->query(
        ("SELECT {chatopgroup}.groupid AS groupid, parent, "
...
function get_groups_($check_away, $operator, $order = null)
{
    ...
    if ($order) {
        switch ($order['by']) {
            ...
            case 'lastseen':
                ...
                break;
            default:
                $orderby = "{chatopgroup}.vclocalname";
        }
        $orderby = sprintf(
            " IF(ISNULL({chatopgroup}.parent),CONCAT('_',%s),'') %s, {chatopgroup}.iweight ",
            $orderby,
            ($order['desc'] ? 'DESC' : 'ASC')
        );
    } else {
    ...
    $query = "SELECT DISTINCT {chatopgroup}.groupid AS groupid, "
        . "{chatopgroup}.parent AS parent, "
        . "vclocalname, vclocaldescription, iweight, "
        . "(SELECT count(*) "
            . "FROM {chatoperatortoopgroup} "
            . "WHERE {chatopgroup}.groupid = {chatoperatortoopgroup}.groupid"
        . ") AS inumofagents, "
        . "(SELECT MIN(:now - dtmlastvisited) AS time "
            . "FROM {chatoperatortoopgroup}, {chatoperator} "
            . "WHERE istatus = 0 "
                . "AND {chatopgroup}.groupid = {chatoperatortoopgroup}.groupid "
                . "AND {chatoperatortoopgroup}.operatorid = {chatoperator}.operatorid" .
        ") AS ilastseen"
        . ($check_away
            ? ", (SELECT MIN(:now - dtmlastvisited) AS time "
                    . "FROM {chatoperatortoopgroup}, {chatoperator} "
    ...
    if ($operator) {
        $query .= ", (SELECT {chatopgroup}.groupid, {chatopgroup}.parent "
            . "FROM {chatopgroup}, {chatoperatortoopgroup} "
            . "WHERE {chatopgroup}.groupid = {chatoperatortoopgroup}.groupid "
                . "AND {chatoperatortoopgroup}.operatorid = :operatorid) i "
            . "WHERE {chatopgroup}.groupid = i.parent "
                . "OR {chatopgroup}.parent = i.groupid "
                . "OR ({chatopgroup}.parent = i.parent AND {chatopgroup}.parent IS NOT NULL) "
                . "OR {chatopgroup}.groupid = i.groupid ";
...
{
    $rows = Database::getInstance()->query(
        "SELECT groupid FROM {chatoperatortoopgroup} WHERE operatorid = ?",
        array($operator_id),
        array('return_rows' => Database::RETURN_ALL_ROWS)
    );
