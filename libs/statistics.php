...
function get_by_page_statistics($start, $end)
{
    ...
    $result = $db->query(
        ("SELECT SUM(visits) as visittimes, "
            ...
            . "SUM(rejectedinvitations) AS rejectedinvitations, "
            . "SUM(ignoredinvitations) AS ignoredinvitations "
        . "FROM {chatvisitedpagestatistics} "
        . "WHERE date >= :start "
            . "AND date < :end "
        . "GROUP BY address"),
...
function calculate_page_statistics()
{
    ...
    try {
        ...
        // Get last record date
        $result = $db->query(
            "SELECT MAX(date) as start FROM {chatvisitedpagestatistics}",
            array(),
            array('return_rows' => Database::RETURN_ONE_ROW)
        );
        ...
        $db_results = $db->query(
            ...
            . "FROM {chatvisitedpage} p, {chatthread} t, "
                . "(SELECT "
                    . "COUNT(*) AS msgs, "
                    . "m.threadid "
                . "FROM {chatmessage} m "
        ...
        foreach ($statistics as $row) {
            ...
            $db->query(
                ("INSERT INTO {chatvisitedpagestatistics} ("
        ...
        $db->query(
            ("UPDATE {chatvisitedpage} SET calculated = 1 "
