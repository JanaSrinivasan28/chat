...
function load_canned_messages($locale, $group_id)
{
    ...
    return $db->query(
        ("SELECT id, vctitle, vcvalue FROM {chatcannedmessage} "
...
function load_canned_message($key)
{
    $db = Database::getInstance();
    $result = $db->query(
        "SELECT vctitle, vcvalue FROM {chatcannedmessage} WHERE id = ?",
        array($key),
...
function save_canned_message($key, $title, $message)
{
    $db = Database::getInstance();
    $db->query(
        "UPDATE {chatcannedmessage} SET vcvalue = ?, vctitle = ? WHERE id = ?",
...
function add_canned_message($locale, $group_id, $title, $message)
{
    $db = Database::getInstance();
    $db->query(
        ("INSERT INTO {chatcannedmessage} (locale,groupid,vctitle,vcvalue) "
            . "VALUES (?, ?, ?, ?)"),
        array(
            $locale,
